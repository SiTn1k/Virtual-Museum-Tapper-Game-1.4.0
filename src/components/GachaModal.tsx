...
import { ARTIFACTS, EPOCHS } from '../data/epochs';
...
function getGachaCost(epochId: EpochId): number {
  ...
  // Epoch 1 = 100, epoch 2 = 200, ..., epoch 12 = 1200
...
export function GachaModal({
  ...
  const handleRoll = useCallback(() => {
    ...
    setCurrentIcon('🎁');
    setPhase('rolling');
  }, [phase, canAfford, onRoll, gachaCost, artifactsByRarity, availableArtifacts]);
        ...
        <div className="p-4 bg-gray-800/50 border-t border-gray-700">
          ...
          {phase === 'result' && (
            <div className="flex gap-2">
              ...
              >
                Ще раз {currency >= gachaCost ? `(${gachaCost} ${epoch.currencyIcon})` : ''}
              </button>
            </div>
          )}
        </div>
